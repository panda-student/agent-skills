...
### 核心原则
将确定性、可重复的执行逻辑封装为命令、工具或脚本，避免在运行时进行重复推理。
...
| 优先级 | 条件 | 封装方式 |
|-------|------|---------|
| P0 | 高频 + 高确定性 | 独立脚本/工具，完整测试覆盖 |
| P1 | 高频 + 中确定性 | npm script + 参数化 |
| P2 | 中频 + 高确定性 | shell 脚本 / Makefile |
| P3 | 中频 + 中确定性 | 文档化的命令序列 |
...
| 逻辑类型 | 封装方式 | 示例 |
|---------|---------|------|
| 代码规范检查 | npm script / shell 脚本 | `npm run lint` |
| 类型检查 | npm script / tsc 命令 | `npm run typecheck` |
| 单元测试 | npm script / jest 命令 | `npm run test` |
| 安全扫描 | 专用工具 / 脚本 | `npm audit` |
| 依赖检查 | npm script / 工具 | `npm run check-deps` |

#### 构建部署类（高优先级）
| 逻辑类型 | 封装方式 | 示例 |
|---------|---------|------|
| 项目构建 | npm script / 构建工具 | `npm run build` |
| 打包发布 | npm script / CI 脚本 | `npm run release` |
| Docker 构建 | shell 脚本 / Makefile | `make docker-build` |
| 环境部署 | shell 脚本 / 部署工具 | `./deploy.sh` |

#### 代码生成类（中优先级）
| 逻辑类型 | 封装方式 | 示例 |
|---------|---------|------|
| 组件生成 | 脚本 / CLI 工具 | `npm run gen:component` |
| API 客户端生成 | OpenAPI 工具 | `npm run gen:api` |
| 类型定义生成 | 代码生成工具 | `npm run gen:types` |
| 模板实例化 | 脚本 / 模板引擎 | `./create-module.sh` |

#### 环境配置类（中优先级）
| 逻辑类型 | 封装方式 | 示例 |
|---------|---------|------|
| 项目初始化 | shell 脚本 / 脚手架 | `./init-project.sh` |
| 依赖安装 | npm script / shell | `npm run setup` |
| 环境变量配置 | 脚本 / 配置工具 | `./setup-env.sh` |
| 数据库迁移 | npm script / CLI | `npm run db:migrate` |

#### Git 操作类（中优先级）
| 逻辑类型 | 封装方式 | 示例 |
|---------|---------|------|
| 规范化提交 | git hooks / commitlint | 自动触发 |
| 分支管理 | shell 脚本 / git alias | `./git-flow.sh` |
| 变更日志生成 | 工具 / 脚本 | `npm run changelog` |
| 版本发布 | npm script / 工具 | `npm run release` |

#### 数据处理类（低优先级）
| 逻辑类型 | 封装方式 | 示例 |
|---------|---------|------|
| 格式转换 | 脚本 / 工具 | `./convert.sh` |
| 数据清洗 | 脚本 / ETL 工具 | `./clean-data.sh` |
| 批量处理 | shell 脚本 | `./batch-process.sh` |

#### 文件操作类（低优先级）
| 逻辑类型 | 封装方式 | 示例 |
|---------|---------|------|
| 批量重命名 | shell 脚本 | `./rename.sh` |
| 文件清理 | npm script / shell | `npm run clean` |
| 目录结构生成 | 脚本 | `./create-structure.sh` |
...
### 封装收益
- **性能提升**：减少模型推理步骤，加快响应速度
- **稳定性提升**：封装逻辑经过测试，减少运行时错误
- **一致性提升**：相同输入产生相同输出
- **可维护性提升**：逻辑集中管理，便于更新
